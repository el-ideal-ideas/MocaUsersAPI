...
try:
    from ujson import dumps as dumps_, loads as json_loads

    json_dumps = partial(dumps_, ensure_ascii=False)
except (ImportError, ModuleNotFoundError):
    from json import dumps as dumps_, loads as json_loads
...
class MocaUsers(MocaNamedInstance, MocaClassCache):
    ...
    async def _init_system_account(self) -> None:
        await self.create_user('Administrator', 'moca-moca-moca', 'moca.administrator', '', True, '-RO-')
        await self.create_user('UserAdmin', 'moca-moca-moca', 'moca.user-admin', '', True, '-MU-')
        await self.create_user('PublicUser', 'moca-moca-moca', 'moca.public', '', True, '')
        await self.create_user('SupportUser', 'moca-moca-moca', 'moca.support', '', True, '')
        await self.create_user('DebugAccountA', 'moca-moca-moca', 'moca.debug-a', '', True, '')
        await self.create_user('DebugAccountB', 'moca-moca-moca', 'moca.debug-b', '', True, '')
    ...
    async def save_profile(self, userid: str, profile: str, access_token: str) -> Tuple[int, str]:
        """
        Return: status_code, response_message.
        Status Code
        -----------
    ...
    async def check_password(self, userid: str, password: str) -> bool:
        ...
        if res[0] != 0:
            return False
        if not self._is_system_account(userid):
            res = self._check_password_format(password)
            if res[0] != 0:
                return False
        pool = await self._mysql.get_aio_pool()
    ...
    async def get_my_login_log(self, userid: str, access_token: str) -> Tuple[int, Union[str, List[List[str]]]]:
        """
        Return: status_code, response_message.
        Status Code
        -----------
        ...
        async with pool.acquire() as con:
            async with con.cursor() as cursor:
                ...
                data = await cursor.fetchall()
                return 0, [[item[0], item[1], item[2], item[3], str(item[4])] for item in data]
    ...
    async def lock_user_account(self, userid: str, password: str, target_userid: str) -> Tuple[int, str]:
        ...
        if res[0] != 0:
            return 2, 'target userid format error.'
        if await self.check_system_account_permission(userid, password, '-MU-|-RO-'):
            pool = await self._mysql.get_aio_pool()
            async with pool.acquire() as con:
                async with con.cursor() as cursor:
    ...
    async def save_user_image(self, userid: str, access_token: str,
                              key: str, image: Union[bytes, str]) -> Tuple[int, str]:
        """
        ...
        if isinstance(image, str):
            ...
            image_data = image
        if len(image_data) > 10485760:  # 10 MB
        ...
        try:
            with Image.open(BytesIO(image_data)) as src:
                data = src.getdata()
                mode = src.mode
                size = src.size
    ...
    async def get_user_image(self, userid: str, key: str) -> Tuple[int, Union[str, bytes]]:
        """
        Return: status_code, response_message.
        Status Code
        -----------
    ...
    async def get_big_icon(self, userid: str) -> Tuple[int, Union[str, bytes]]:
        """
        Return: status_code, response_message.
        Status Code
        -----------
        ...
        63: user is must only contains ascii characters.
        """
        return await self.get_user_image(userid, 'moca-user-icon-big')
    ...
    async def get_small_icon(self, userid: str) -> Tuple[int, Union[str, bytes]]:
        """
        Return: status_code, response_message.
        Status Code
        -----------
        ...
        63: user is must only contains ascii characters.
        """
        return await self.get_user_image(userid, 'moca-user-icon-small')

    async def save_user_file(self, userid: str, access_token: str,
                             key: str, data: Union[str, bytes]) -> Tuple[int, str]:
        """
        Return: status_code, response_message.
        Status Code
        -----------
        ...
        elif len(data_) > 268435456:  # 256 MB
        ...
        async with open(filename, mode='wb') as file:
            await file.write(compress(data_))
            return 0, 'success.'
    ...
    async def get_messages(self, userid: str, access_token: str, start: int, limit: int) \
        ...
        async with pool.acquire() as con:
            async with con.cursor() as cursor:
                await cursor.execute(MocaUsers._GET_MESSAGES, (userid, start, limit))
                data = await cursor.fetchall()
                if len(data) > 0:
                    return 0, list(data)
                else:
    ...
    async def get_user_email_list(self, userid_list: List[str]) -> Dict[str, str]:
    ...
    async def send_email_to_users(self, userid_list: List[str], title: str, body: str, type_='html') -> List[str]:
        ...
        The maximum length of title is 64, the maximum length of body is 16384.
        The maximum length of userid_list is 1024.
        """
        if (not isinstance(title, str)) or (not isinstance(body, str)) or (type_ not in ('html', 'plain')) \
                or (len(title) > 64) or (len(body) > 16384) or (not isinstance(userid_list, list))\
                or (len(userid_list) > 1024):
            return []
        email_list: Dict[str, str] = await self.get_user_email_list(userid_list)
        success_list: List[str] = []
        for userid, email in email_list.items():
            try:
                await self._mail.send_aio_mail([email], body, title, type_)
                success_list.append(userid)
            except SMTPException:
                pass
        return success_list

    async def get_user_phone_number(self, userid_list: List[str]) -> Dict[str, str]:
        """The maximum length of userid_list is 1024."""
        if (not isinstance(userid_list, list)) or (len(userid_list) > 1024):
            return {}
        pool = await self._mysql.get_aio_pool()
    ...
    async def send_sms_to_users(self, userid_list: List[str], body: str) -> List[str]:
        ...
        if (not isinstance(userid_list, list)) or (not isinstance(body, str)) \
                or (len(body) > 1024) or (len(userid_list) > 1024):
            return []
        phone_list: Dict[str, str] = await self.get_user_phone_number(userid_list)
        success_list: List[str] = []
        for userid, phone in phone_list.items():
            try:
                self._sms.send_sms(body, phone)
                success_list.append(userid)
    ...
    async def get_my_user_data(self, userid: str, access_token: str, storage: int) -> Tuple[int, Any]:
        ...
        async with pool.acquire() as con:
            async with con.cursor() as cursor:
                ...
                if len(res) > 0:
                    data = loads(res[0][0])
                    if res[0][0] is None or len(res[0][0]) < 4096:
                        await self._redis.save(
                            f'moca-storage{storage}-cache-{userid}',
                            data,
                            600  # 10 minutes.
    ...
    async def share_file(self, userid: str, access_token: str, filename: str, data: Union[bytes, str],
                         protection: str = '', time_limit: int = 0, info: str = '') -> Tuple[int, str]:
        """
        Return: status_code, response_message.
        Status Code
        -----------
        ...
        async with open(str(path.joinpath('data')), mode='wb') as data_file:
            await data_file.write(compress(data_))
