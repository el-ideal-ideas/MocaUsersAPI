...
try:
    from ujson import dump as dump_, dumps as dumps_
    dump = partial(dump_, ensure_ascii=False)
    dumps = partial(dumps_, ensure_ascii=False)
except (ImportError, ModuleNotFoundError):
    from json import dump as dump_, dumps as dumps_

    # This is done in order to ensure that the JSON response is
    # kept consistent across both ujson and inbuilt json usage.
    dump = partial(dump_, separators=(",", ":"), ensure_ascii=False)
    dumps = partial(dumps_, separators=(",", ":"), ensure_ascii=False)
...
def wget(url: str, filename: Union[Path, str], timeout: int = 5) -> bool:
    """
    Download a file from url.
    if the http status code is 200 return true.
    """
    try:
        res: Response = get(url, allow_redirects=True, timeout=timeout)
...
async def aio_wget(url: str, filename: Union[Path, str], timeout: int = 5) -> bool:
    """
    Download a file from url.
    if the http status code is 200 return true.
    """
...
def wcheck(url: str, timeout: int = 5) -> bool:
...
async def aio_wcheck(url: str, timeout: int = 5) -> bool:
    """If the http status is 200 return true."""
    try:
        async with ClientSession() as session:
            async with session.get(url, allow_redirects=True, timeout=timeout) as res:
...
def wstatus(url: str, timeout: int = 5) -> int:
...
async def aio_wstatus(url: str, timeout: int = 5) -> int:
    """return the http status code."""
    try:
        async with ClientSession() as session:
            async with session.get(url, allow_redirects=True, timeout=timeout) as res:
...
def dumps_json_beautiful(data: Any) -> str:
    ...
    return dumps(data, ensure_ascii=False, indent=4, sort_keys=True)
